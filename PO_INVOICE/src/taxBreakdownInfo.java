...
public class taxBreakdownInfo {
	...
	public static JSONArray getTaxBreakdownInfo() throws Exception {
		...
		try {
			...
			for (int i = 0; i < countValue; i++) {
				if (!taxPercent_list.contains(","
						+ items.get("TAX_PERCENT1")[i] + ",")) {
					taxPercent_list += items.get("TAX_PERCENT1")[i] + ",";
					Long[] amount = new Long[2];
					amount[0] = Long.parseLong(items.get("AMOUNT")[i]);
					amount[1] = Long.parseLong(items.get("TAX_AMOUNT")[i]);
					map_tax_rate.put(items.get("TAX_PERCENT1")[i], amount);
				} else {
					Long[] amount = map_tax_rate
							.get(items.get("TAX_PERCENT1")[i]);
					amount[0] += Long.parseLong(items.get("AMOUNT")[i]);
					amount[1] += Long.parseLong(items.get("TAX_AMOUNT")[i]);
					map_tax_rate.put(items.get("TAX_PERCENT1")[i], amount);
				}
			}
			String[] tax_pct = taxPercent_list.substring(1,
					taxPercent_list.length()).split(",");
