...
public class create_json_string {
	static public String invoice_draft(String header_id) {
		...
		try {
			...
			// Construct Query
			column_name = "BANK_ID"// invoiceSeries
			;
			table_name = "OM.OM_HEADERS";
			filter_statement = " where" + " ORDER_NUMBER = \'"
					+ invoice_info.get("SO_NUMBER") + "\'";
	...
	static public String invoice(String header_id) {
		...
		try {
			HashMap<String, String> invoice_info = new HashMap<String, String>();
			invoice_info.put("invoiceNo", "");
			invoice_info.put("fileType", "");
			invoice_info.put("templateCode", "");
			invoice_info.put("transactionUuid", "");
			invoice_info.put("supplierTaxCode", "");
			
			JSONObject general = generalInfo.getGeneralInfo(header_id);
			JSONObject seller = sellerInfo.getSellerInfo("sellerId");
			
			invoice_info.put("supplierTaxCode", seller.getString("sellerTaxCode"));
			invoice_info.put("transactionUuid", general.getString("transactionUuid"));
			invoice_info.put("templateCode", general.getString("templateCode"));
			
			val.put("supplierTaxCode", invoice_info.get("supplierTaxCode"));
			val.put("transactionUuid", invoice_info.get("transactionUuid"));
			val.put("templateCode", invoice_info.get("templateCode"));
			val.put("invoiceNo", "InvoiceNo");
			val.put("fileType", "ZIP");
		} catch (Exception ex) {
			ex.printStackTrace();
		}		
		return val.toString(1);
